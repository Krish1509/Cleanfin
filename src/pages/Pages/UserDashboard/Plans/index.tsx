...
const Plans = () => {
    ...
    const [subscriptionPlans, setSubscriptionPlans] = useState<ISubPlan[] | undefined>(undefined);
            ...
            <Row>
                <Col sm={12}>
                    ...
                    <div className={`multi-collapse collapse show`}>
                        <Row>
                            {subscriptionPlans
                                ?.filter((plan: ISubPlan) => plan?.period === (show ? 'monthly' : 'yearly'))
                                ?.map((items: ISubPlan, index: number) => {
                                    return (
                                        <Col md={6} xxl={4} key={index}>
                                            <Card className="price-card">
                                                <CardBody
                                                    className={`price-head ${items?.item?.currency === 'INR' ? 'bg-light-secondary' : 'bg-light-primary'
                                                        }`}
                                                >
                                                    <h5 className={items?.item?.active ? 'text-secondary' : 'text-muted'}>
                                                        {items?.item?.name}
                                                    </h5>
                                                    <h2
                                                        className={`price-price ${items?.item?.currency === 'INR' ? 'text-secondary' : 'text-primary'
                                                            }`}
                                                    >
                                                        {`₹${items?.item?.amount / 100}`} <span>{`/ ${items?.period}`}</span>
                                                    </h2>
                                                    <div
                                                        className={`price-icon ${items?.item?.currency === 'INR' ? 'bg-light-secondary' : 'bg-light-primary'
                                                            }`}
                                                    >
                                                        <i
                                                            className={`${items?.item?.type === 'plan'
                                                                ? 'ph-duotone ph-buildings'
                                                                : 'ph-duotone ph-cogs'
                                                                } ${items?.item?.active ? 'text-secondary' : 'text-muted'}`}
                                                        ></i>
                                                    </div>
                                                </CardBody>
                                                <CardBody>
                                                    ...
                                                    <div className="d-grid" style={{ gap: 5 }}>
                                                        <button
                                                            className={`btn ${items?.item?.active ? 'btn-secondary' : 'btn-muted'}`}
                                                            onClick={() => handlePurchase(items?.id)}
                                                        >
                                                            {items?.notes?.length > 0 ? items?.notes[0] : 'Upgrade Plan'}
                                                        </button>
                                                        <button
                                                            className={`btn ${items?.item?.active ? 'btn-outline-secondary' : 'btn-outline-muted'}`}
